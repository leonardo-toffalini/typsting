...
= Notes
#theorem[
  Let $Omega subset.eq RR^3$ be a domain, and let a constant $mu > 0$ and an
  element $f in H^*$ be given. Then $exists (u, p) in H^1_0 (Omega) times L^2_0
  (Omega)$ such that
  $
...
#proof[
  ...
  *1. Finding a weak solution.*
  ...
  $
    ...
    &<=^"C.S.B" (sum_(j=1)^n sum_(i=1)^n norm(partial_i u_j)_(L^4))^(1/2) (sum_(j=1)^n sum_(i=1)^n norm(w_i)_(L^4) norm(v_j)_(L^4))^(1/2) \
    &= norm(nabla u)_(L^4) (sum_(i=1)^n norm(w_i)_(L^4))^(1/2) (sum_(j=1)^n norm(v_j)_(L^4))^(1/2) \
    &<= c norm(u)_(H^1_0) norm(w)_(L^4) norm(v)_(L^4).
  $

  #theorem(title: "Sobolev embedding")[
    $W^(m, p) (Omega) arrow.hook L^(p^*) (Omega)$ if $1/p^* = 1/p - m/N$ if $m
    < N/p$ and $Omega subset.eq RR^N$.
  ]

  #corollary[
    $H^1 (Omega) = W^(1, 2) (Omega) quad m = 1, quad p = 2, quad N = 3$.
  ...
  $
  so $b$ is a bounded trilinear function over $(H^1_0 (Omega))^3$, which also
  ...
  *2. Technical preliminary*
  ...
  $
    b(w, v, v) &= integral_Omega w_j (partial_j v_i) v_i \
    &= 1/2 integral_Omega w_j partial_j (v_i^2)
  $
  ...
  *3. Proving the existence of a solution*
  ...
  Let $V := {v in H^1_0 (Omega): nabla dot u |_Omega = 0}$. Since $V$ is a
  ...
  says that a solution $F^n (w) in V^n$ uniquely exists.
  ...
  $
    lim_(n -> oo) (overbrace(a(u^n, v^n) + b(u^n, u^n, v^n) - l(v^n), 0 forall v^n in V^n)) = a(u, v) + b(u, u, v) - l(v)
  $
